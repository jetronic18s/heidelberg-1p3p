...
OTADATA_BIN="${ROOT_DIR}/build/ota_data_initial.bin"
APP_BIN="${ROOT_DIR}/build/heidelberg-1p3p.bin"
...
if [[ -f "${BOOTLOADER_BIN}" && -f "${PARTITION_BIN}" && -f "${OTADATA_BIN}" && -f "${APP_BIN}" ]]; then
  ...
  echo "Prebuilt binaries not found under ${ROOT_DIR}/build, falling back to idf.py flash."
  ...
  if [[ -n "${IDF_VER}" ]]; then
    ...
    if [[ "$(printf '%s\n' "${MIN_VER}" "${IDF_VER}" | sort -V | head -n1)" != "${MIN_VER}" ]]; then
      echo "Detected ESP-IDF ${IDF_VER}, but this project requires >= ${MIN_VER}." >&2
      echo "Use a newer export, e.g.: . /tmp/esp-idf-5.5.4/export.sh" >&2
      exit 1
    fi
  fi
