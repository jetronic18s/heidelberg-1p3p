...
static esp_err_t jl1101_update_link_duplex_speed(phy_jl1101_t *jl1101)
{
    ...
    PHY_CHECK(eth->phy_reg_read(eth, jl1101->addr, ETH_PHY_BMSR_REG_ADDR, &(bmsr.val)) == ESP_OK,
    ...
    if (jl1101->link_status != link) {
        ...
        if (link == ETH_LINK_UP) {
            PHY_CHECK(eth->phy_reg_read(eth, jl1101->addr, ETH_PHY_BMCR_REG_ADDR, &(bmcr.val)) == ESP_OK,
...
static esp_err_t jl1101_reset(esp_eth_phy_t *phy)
{
    ...
    PHY_CHECK(eth->phy_reg_write(eth, jl1101->addr, ETH_PHY_BMCR_REG_ADDR, bmcr.val) == ESP_OK,
...
static esp_err_t jl1101_reset_hw(esp_eth_phy_t *phy)
{
    ...
    if (jl1101->reset_gpio_num >= 0) {
        esp_rom_gpio_pad_select_gpio((gpio_num_t)jl1101->reset_gpio_num);
        gpio_set_direction((gpio_num_t)jl1101->reset_gpio_num, GPIO_MODE_OUTPUT);
        gpio_set_level((gpio_num_t)jl1101->reset_gpio_num, 0);
        esp_rom_delay_us(100); // insert min input assert time
        gpio_set_level((gpio_num_t)jl1101->reset_gpio_num, 1);
    }
    return ESP_OK;
}
...
static esp_err_t jl1101_negotiate(esp_eth_phy_t *phy)
{
    ...
    bmcr_reg_t bmcr;
    bmcr.val = 0; // Initialize to zero
    bmcr.val |= (1 << 12); // Enable Auto Negotiation (Bit 12)
    bmcr.val |= (1 << 9);  // Restart Auto Negotiation (Bit 9)
    PHY_CHECK(eth->phy_reg_write(eth, jl1101->addr, ETH_PHY_BMCR_REG_ADDR, bmcr.val) == ESP_OK,
              "write BMCR failed", err);
...
static esp_err_t jl1101_get_addr(esp_eth_phy_t *phy, uint32_t *addr)
{
    ...
    jl1101 = __containerof(phy, phy_jl1101_t, parent);
    *addr = jl1101->addr;
    return ESP_OK;
err:
    return ESP_ERR_INVALID_ARG;
...
static esp_err_t jl1101_advertise_pause_ability(esp_eth_phy_t *phy, uint32_t ability)
{
    ...
    anar_reg_t anar;
    PHY_CHECK(eth->phy_reg_read(eth, jl1101->addr, ETH_PHY_ANAR_REG_ADDR, &(anar.val)) == ESP_OK,
    ...
    PHY_CHECK(eth->phy_reg_write(eth, jl1101->addr, ETH_PHY_ANAR_REG_ADDR, anar.val) == ESP_OK,
...
static esp_err_t jl1101_init(esp_eth_phy_t *phy)
{
    ...
    /* Check PHY ID */
    PHY_CHECK(eth->phy_reg_read(eth, jl1101->addr, ETH_PHY_IDR1_REG_ADDR, &(id1.val)) == ESP_OK,
              "read ID1 failed", err);
    PHY_CHECK(eth->phy_reg_read(eth, jl1101->addr, ETH_PHY_IDR2_REG_ADDR, &(id2.val)) == ESP_OK,
              "read ID2 failed", err);
    PHY_CHECK(id1.oui_msb == 0x937C && id2.oui_lsb == 0x10 && id2.vendor_model == 0x2,
              "wrong chip ID", err);
    return ESP_OK;
err:
    return ESP_FAIL;
...
{
    phy_jl1101_t *jl1101 = NULL;
    PHY_CHECK(config, "can't set phy config to null", err);
    jl1101 = static_cast<phy_jl1101_t*>(calloc(1, sizeof(phy_jl1101_t)));
