...
static esp_err_t jl1101_reset(esp_eth_phy_t *phy)
{
    ...
    PHY_CHECK(eth->phy_reg_write(eth, jl1101->addr, ETH_PHY_BMCR_REG_ADDR, bmcr.val) == ESP_OK, "write BMCR failed",
              err);
    uint32_t to = 0;
    ...
    PHY_CHECK(to < jl1101->reset_timeout_ms / 10, "reset timeout", err);
    return ESP_OK;
err:
    return ret;
}
...
static esp_err_t jl1101_negotiate(esp_eth_phy_t *phy)
{
    ...
    if ((to >= jl1101->autonego_timeout_ms / 100) && (jl1101->link_status == ETH_LINK_UP)) {
...
static esp_err_t jl1101_advertise_pause_ability(esp_eth_phy_t *phy, uint32_t ability)
{
    ...
    PHY_CHECK(eth->phy_reg_write(eth, jl1101->addr, ETH_PHY_ANAR_REG_ADDR, anar.val) == ESP_OK, "write ANAR failed",
              err);
    return ESP_OK;
err:
    return ret;
...
static esp_err_t jl1101_init(esp_eth_phy_t *phy)
{
    ...
    /* Check PHY ID */
    PHY_CHECK(eth->phy_reg_read(eth, jl1101->addr, ETH_PHY_IDR1_REG_ADDR, &(id1.val)) == ESP_OK, "read ID1 failed",
              err);
    PHY_CHECK(eth->phy_reg_read(eth, jl1101->addr, ETH_PHY_IDR2_REG_ADDR, &(id2.val)) == ESP_OK, "read ID2 failed",
              err);
    if (!(id1.oui_msb == 0x7 && id2.oui_lsb == 0x356 && id2.vendor_model == 0x08)) {
        ESP_LOGW(TAG, "unexpected PHY ID (ouimsb=0x%x, ouilsb=0x%x, model=0x%x), continuing",
                 id1.oui_msb, id2.oui_lsb, id2.vendor_model);
    }
    return ESP_OK;
err:
    return ret;
...
esp_eth_phy_t *esp_eth_phy_new_jl1101(const eth_phy_config_t *config)
{
    ...
    jl1101 = (phy_jl1101_t *) calloc(1, sizeof(phy_jl1101_t));
