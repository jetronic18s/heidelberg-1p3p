...
static esp_err_t handleStatusGet(httpd_req_t *req)
{
  ...
  sendTableRow(body, "Energy since PowerOn (Wh)", (uint32_t)(s_phase_switch->getInputRegister(15) << 4 | s_phase_switch->getInputRegister(16)));
  sendTableRow(body, "Energy since Installation (Wh)", (uint32_t)(s_phase_switch->getInputRegister(17) << 4 | s_phase_switch->getInputRegister(18)));
  body += "<tr><td>&nbsp;</td><td></td></tr>";
...
static esp_err_t handleDebugPost(httpd_req_t *req)
{
  ...
  sendResponseHeader(body, "Debug");
  body += "<pre>";
  ModbusMessage answer = s_phase_switch->sendRtuRequest((uint8_t)strtoul(slaveId.c_str(), nullptr, 10),
                                                        (uint8_t)strtoul(func.c_str(), nullptr, 10),
                                                        (uint16_t)strtoul(reg.c_str(), nullptr, 10),
                                                        (uint16_t)strtoul(count.c_str(), nullptr, 10));
  body += "</pre>";

  auto error = answer.getError();
  if (error == SUCCESS) {
    body += "<span>Answer: 0x";
    int payloadCount = answer[2];
    for (int i = 0; i < payloadCount; i++) {
      appendf(body, "%02x", answer[i + 3]);
    }
    body += "</span>";
  } else {
    appendf(body, "<span class=\"e\">Error: %#02x (%s)</span>", error, ErrorName(error).c_str());
  }
...
static esp_err_t handleWifiSetupGet(httpd_req_t *req)
{
  ...
  body += "</select></td></tr>";
  body += "<tr><td><label for=\"ssid\">WiFi SSID</label></td><td><input type=\"text\" id=\"ssid\" name=\"ssid\" required></td></tr>";
  body += "<tr><td><label for=\"pass\">WiFi Password</label></td><td><input type=\"password\" id=\"pass\" name=\"pass\"></td></tr>";
  body += "</table><p></p><button class=\"r\">Save</button></form><p></p>";
...
std::string ErrorName(Modbus::Error code)
{
  switch (code)
  {
